...
describe("Lib / Helper functions", function()
  ...
  describe("Glob pattern matching", function()
    it("glob_to_pattern converts * correctly", function()
      ...
      assert.equals("^/home/user/[^/]*$", pattern)
    end)
    ...
    it("glob_to_pattern escapes Lua pattern special characters", function()
      local pattern = Lib.glob_to_pattern("/path/with-dashes/and.dots/and%percent")
      -- Should escape -, ., and %
      assert.is_not_nil(string.find(pattern, "%-"))
      assert.is_not_nil(string.find(pattern, "%."))
      assert.is_not_nil(string.find(pattern, "%%"))
    end)

    it("glob_to_pattern handles tilde expansion", function()
      -- Normalize home the same way glob_to_pattern does (forward slashes on Windows)
      local home = vim.fs.normalize(vim.fn.expand("~"))
      local pattern = Lib.glob_to_pattern("~/.config/*")
      -- Should start with home directory (escaped for pattern matching)
      assert.is_not_nil(string.find(pattern, "^%^" .. vim.pesc(home)))
    end)

    it("glob_to_pattern removes trailing slashes", function()
      local pattern1 = Lib.glob_to_pattern("/home/user/")
      local pattern2 = Lib.glob_to_pattern("/home/user")
      assert.equals(pattern1, pattern2)
    end)

    it("path_matches_glob matches single-level wildcards", function()
      assert.True(Lib.path_matches_glob("/home/user/projects", "/home/user/*"))
      assert.True(Lib.path_matches_glob("/mnt/drive1/docs", "/mnt/*/docs"))
      assert.False(Lib.path_matches_glob("/home/user/projects/subdir", "/home/user/*"))
    end)

    it("path_matches_glob matches question mark wildcards", function()
      assert.True(Lib.path_matches_glob("/home/user/a", "/home/user/?"))
      assert.True(Lib.path_matches_glob("/home/user/1", "/home/user/?"))
      assert.False(Lib.path_matches_glob("/home/user/ab", "/home/user/?"))
      assert.False(Lib.path_matches_glob("/home/user/a/b", "/home/user/?"))
    end)

    it("path_matches_glob handles exact paths (no wildcards)", function()
      assert.True(Lib.path_matches_glob("/home/user/projects", "/home/user/projects"))
      assert.False(Lib.path_matches_glob("/home/user/projects/sub", "/home/user/projects"))
      assert.False(Lib.path_matches_glob("/home/user/other", "/home/user/projects"))
    end)

    it("path_matches_glob handles paths with trailing slashes", function()
      assert.True(Lib.path_matches_glob("/home/user/projects/", "/home/user/*"))
      assert.True(Lib.path_matches_glob("/home/user/projects", "/home/user/*/"))
    end)

    it("path_matches_glob handles tilde in patterns", function()
      local home = vim.fn.expand("~")
      assert.True(Lib.path_matches_glob(home .. "/projects", "~/*"))
      assert.True(Lib.path_matches_glob(home .. "/.config", "~/.*"))
    end)

    it("path_matches_glob handles multiple wildcards", function()
      assert.True(Lib.path_matches_glob("/mnt/drive1/project/docs", "/mnt/*/project/*"))
      assert.False(Lib.path_matches_glob("/mnt/drive1/docs", "/mnt/*/project/*"))
    end)

    it("path_matches_glob handles dot files with wildcards", function()
      assert.True(Lib.path_matches_glob("/home/user/.config", "/home/user/.*"))
      assert.True(Lib.path_matches_glob("/home/user/.bashrc", "/home/user/.*"))
      assert.False(Lib.path_matches_glob("/home/user/config", "/home/user/.*"))
    end)

    it("path_matches_glob normalizes paths", function()
      assert.True(Lib.path_matches_glob("/home/user/./projects", "/home/user/*"))
      assert.True(Lib.path_matches_glob("/home/user/sub/../projects", "/home/user/*"))
    end)
  end)
end)
